const { listTask } = require("../models"); //import listTalk in models
...
class Task {
  ...
  static async getOneTask(req, res, next) {
    try {
      ...
      if (!data) {
        return next({ message: "The task not found", statusCode: 404 });
      }
  ...
  static async updateTask(req, res, next) {
    try {
      ...
      //If there is no the data updated
      if (updateData[0] === 0) {
        return res.status(404).json({ errors: ["The task not found"] });
      }
  ...
  static async deleteTask(req, res, next) {
    try {
      ...
      //If success
      res.status(200).json({ message: "Success delete of the task" });
    } catch (error) {
      next(error);
    }
  }
